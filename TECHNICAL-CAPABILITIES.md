...
2. **`confirm: true` parameter fallback**: used only when the client doesn't declare the `elicitation` capability (or the elicitation call itself fails). The affected tool call is intercepted and returns guidance (`src/utils/guidance.ts`: `getConfirmationRequiredGuidance` / `getConfirmationDeclinedGuidance`) instead of reaching Rundeck unless `confirm: true` is explicitly set. This depends on the calling agent having actually obtained explicit user approval first — a weaker guarantee than elicitation, since the agent controls the flag itself.
