...
// A single Server instance, built once: this stdio process serves exactly
// one connection for its whole lifetime, so serveStdio() (below) only ever
// calls its factory once — the same instance transparently serves both the
// 2025-era protocol and 2026-07-28 via the SDK's legacy shim.
...
try {
  const stdioHandle = serveStdio(() => server, {
    // Out-of-band errors from the opening/era-classification exchange
    // itself (e.g. a malformed 2026-07-28 envelope claim) happen before any
    // Server instance is pinned, so they never reach that instance's own
    // onerror — without this, they're answered correctly on the wire but
    // invisible in our logs.
