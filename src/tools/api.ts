...
export const rundeckApiCallSchema = z.object({
  ...
  userHasProvidedConfirmation: z.boolean()
    .optional()
    .default(false)
    .describe(
      "Set this to true ONLY if literally true: the user has, in this conversation, already given " +
      "explicit approval for this specific 'DELETE' or runner-'regenerateCreds' call, AND a prior " +
      "call to this same tool told you elicitation wasn't available or didn't go through. Do not " +
      "set it on a first attempt, and do not set it just because you think the action is probably " +
      "fine or the user seems likely to approve — call the tool first with this omitted; if the " +
      "connected client supports MCP elicitation, the server prompts the user itself and this " +
      "field has no effect at all. Only ask the user yourself, then set this to true and retry, if " +
      "the response explicitly tells you to."
    ),
});
