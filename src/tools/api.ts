...
export const rundeckApiCallSchema = z.object({
  ...
  userHasProvidedConfirmation: z.boolean()
    ...
    .describe(
      ...
      "call to this same tool told you to get that confirmation. Do not set it on a first attempt, " +
      "and do not set it just because you think the action is probably fine or the user seems " +
      "likely to approve — call the tool first with this omitted. If the connected client supports " +
      "MCP elicitation, the server prompts the user itself and this field has no effect at all. " +
      "Only ask the user yourself, then set this to true and retry, if the response explicitly " +
      "tells you to."
    ),
});
