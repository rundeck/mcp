...
export const rundeckApiCallSchema = z.object({
  ...
  skipConfirmation: z.boolean()
    ...
    .describe(
      ...
      "(omit this field) and wait for the outcome. Only set this to true when method is 'DELETE', " +
      "or for a POST to a runner's 'regenerateCreds' endpoint (which revokes its current " +
      "credentials), AND the tool's response says elicitation wasn't available or didn't go " +
      "through — and only after the user has explicitly approved that specific action at that " +
      "point. Never inferred or set proactively on the agent's own judgment."
    ),
});
